...
# create index route
@app.route('/', methods=['POST', 'GET']) # GET default
def index():
    if request.method == 'POST':
        ...
        tasks = Todo.query.order_by(Todo.date_created).all() # or first() for top
...
def update(id):
    ...
    if request.method == 'POST':
        task.content = request.form['content'] # setting current task (line 48) content to one form input
        try:
            db.session.commit()
            return redirect('/')
        except:
